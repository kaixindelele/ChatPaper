...
@tenacity.retry(wait=tenacity.wait_exponential(multiplier=1, min=4, max=10),
                    ...
                    reraise=True)
def chat_translate_part(text, key, title=False, domain="", tokenizer_gpt35=None, task="翻译", base_url=None):
    ...
    info['response_time'] = response.response_ms / 1000.0  # MIGRATION-REVIEW: response-objects
    return info

@tenacity.retry(wait=tenacity.wait_exponential(multiplier=1, min=4, max=10),
                    stop=tenacity.stop_after_attempt(8),
                    reraise=True)
def chat_check_domain(text, key, base_url=None):
    client = OpenAI(api_key=key, base_url=base_url)
    messages = [
            {"role": "system",
                "content": "You are now a professional Science and technology editor"},
...
def main(root_path, pdf_path, base_url, key, task="翻译"):
    ...
    if "title" in paper_pdf.keys() and "abstract" in paper_pdf.keys():
        ...
        return_dict = chat_check_domain(text, key, base_url=base_url)
    ...
    if "title" in paper_pdf.keys():
        ...
        return_dict = chat_translate_part(text, key, title=True, domain=domains, tokenizer_gpt35=tokenizer_gpt35, base_url=base_url)
    ...
    if "abstract" in paper_pdf.keys():
        ...
        return_dict = chat_translate_part(text, key, domain=domains, tokenizer_gpt35=tokenizer_gpt35, base_url=base_url)
        result = return_dict['result']
        cur_str = "\n"
        cur_str += result
        cur_str += "\n"       
    ...
    for section_index, section_name in enumerate(paper_pdf['section_names']):
        ...
        if len(paper_pdf['section_texts'][section_index])>0:
            ...
            return_dict = chat_translate_part(text, key, domain=domains, tokenizer_gpt35=tokenizer_gpt35, task=task, base_url=base_url)
            result = return_dict['result']
            cur_str = "\n"
            cur_str += result
            cur_str += "\n"
